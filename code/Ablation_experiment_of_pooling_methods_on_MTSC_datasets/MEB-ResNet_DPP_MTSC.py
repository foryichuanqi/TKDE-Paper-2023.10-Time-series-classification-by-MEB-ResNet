...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov  5 22:56:50 2022
...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov  5 22:52:23 2022
...
nb_epochs = 250
run_times=10
method_name='R_2_multi_scale_DPP_RESNET_MTSC'
    ...
    for i in range(run_times):
        ...
        class DPP(Layer):
            def __init__(self,channels,strides):
                ...
                self.ave3=keras.layers.AveragePooling2D((self.stride,1), strides=self.stride)
                self.ave4=keras.layers.AveragePooling2D((self.stride,1), strides=self.stride)
                
            def call(self, I, training=None):
                I_1=self.ave1(I)
                It=keras.layers.UpSampling2D(size=(self.stride, 1), data_format=None)(I_1)
                I_c=keras.layers.Cropping2D(cropping=((0, int(I.shape[1]-It.shape[1])), (0, 0) ), data_format=None)(I)
                print(I_c.shape)
                print('I_cccc')
                It_sub=keras.layers.subtract([I_c,It])
                x=Lambda(lambda x: (x**2)+1e-3)(It_sub)
        
                I_2=self.ave2(x)
                xn=keras.layers.UpSampling2D(size=(self.stride, 1), data_format=None)(I_2)
                xn_1=Lambda(lambda x: 1/x)(xn)
                w=keras.layers.multiply([x,xn_1])
                kp=self.ave3(w)
                I_w=keras.layers.multiply([I_c,w])
                
                Iw=self.ave4(I_w)
                
                kp_1=Lambda(lambda x: 1/x)(kp)
                
                output=keras.layers.multiply([Iw,kp_1])
...
#            exec('x_{}={}'.format(i,keras.layers.Input((65,1,1,))),globals())
            ...
            multi_scale_1=DPP(input_shape[2],1)(x_0)
            multi_scale_2=DPP(input_shape[2],2)(x_0)
            multi_scale_3=DPP(input_shape[2],3)(x_0)
            multi_scale_4=DPP(input_shape[2],4)(x_0)
            multi_scale_5=DPP(input_shape[2],5)(x_0)
