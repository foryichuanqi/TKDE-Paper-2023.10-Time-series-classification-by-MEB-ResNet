...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov  4 12:33:29 2022
...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov  4 12:16:55 2022
...
nb_epochs = 250
run_times=10
method_name='R_2_multi_scale_Projection_LIP_RESNET_MTSC'
...
for (num,each) in enumerate(flist[run_begin_index:]):
    ...
    for i in range(run_times):
        ...
        # class Projection_LIP(Layer):
        #     # 去替代标准的 layers.Dense()
        #     def __init__(self, channels,strides):
        #         super(Projection_LIP, self).__init__()
        ...
        #         x=keras.layers.Conv2D(self.channel, 1, strides=1, use_bias=False)(inputs) ######计算logits的kernelsize 不能设置为channel，因为时间序列数据集的channel 可能超过时间序列的长度 ，设置为8。第二个channel为卷积核的大小。 应该为8  x=keras.layers.Conv2D(channels, channels, strides=3, padding=pad, bias=False)(inputs)
        #         x=InstanceNormalization(axis=3)(x)
        #         x=keras.activations.sigmoid(x)
        #         logit=Lambda(lambda x: x * 12)(x)
        #         weight=Lambda(lambda x: K.exp(x))(logit)
        #         inputs_mul_weight=keras.layers.multiply([weight,inputs])
        #         x_up=keras.layers.AveragePooling2D((self.stride,1), strides=self.stride)(inputs_mul_weight)
        #         x_down=keras.layers.AveragePooling2D((self.stride,1), strides=self.stride)(weight)
        #         x_down_1=Lambda(lambda x: 1/x)(x_down)
        #         output=keras.layers.multiply([x_up,x_down_1])
        ...
        class Projection_LIP(Layer):
            ...
            def __init__(self, channels,strides):
                ...
                self.channel = channels
                self.stride = strides
                self.conv1=keras.layers.Conv2D(self.channel, 1, strides=1, use_bias=False)
                # self.conv2=keras.layers.Conv2D(self.channel, 3, strides=1,padding='same', use_bias=False)
                # self.conv3=keras.layers.Conv2D(self.channel, 1, strides=1, use_bias=False)
                self.IN=InstanceNormalization(axis=3)
                # self.SIG=keras.activations.sigmoid()
                # self.lam1=Lambda(lambda x: x * 12)
                # self.lam2=Lambda(lambda x: K.exp(x))
                # self.mul1=keras.layers.multiply()
                self.ave1=keras.layers.AveragePooling2D((self.stride,1), strides=self.stride)
                self.ave2=keras.layers.AveragePooling2D((self.stride,1), strides=self.stride)
                # self.lam3=Lambda(lambda x: 1/x)
                # self.mul1=keras.layers.multiply()
            ...
            # def call(self, inputs):
                ...
                weight=Lambda(lambda x: K.exp(x))(logit)
                inputs_mul_weight=keras.layers.multiply([weight,inputs])
                x_up=self.ave1(inputs_mul_weight)
                x_down=self.ave2(weight)
                x_down_1=Lambda(lambda x: 1/x)(x_down)
                output=keras.layers.multiply([x_up,x_down_1])    
...
#            exec('x_{}={}'.format(i,keras.layers.Input((65,1,1,))),globals())
            ...
            multi_scale_1=Projection_LIP(input_shape[2],1)(x_0)
            multi_scale_2=Projection_LIP(input_shape[2],2)(x_0)
            multi_scale_3=Projection_LIP(input_shape[2],3)(x_0)
            multi_scale_4=Projection_LIP(input_shape[2],4)(x_0)
            multi_scale_5=Projection_LIP(input_shape[2],5)(x_0)
