...
nb_epochs = 250
run_times=10
method_name='R_2_multi_scale_BottleneckLIP_RESNET_MTSC'
...
for (num,each) in enumerate(flist[run_begin_index:]):
    ...
    for i in range(run_times):
        ...
        class BottleneckLIP_LIP(Layer):
            # 去替代标准的 layers.Dense()
            def __init__(self, channels,strides):
                super(BottleneckLIP_LIP, self).__init__()
            ...
            # def call(self, inputs):
                ...
                # x=keras.layers.Conv2D(self.channel, 1, strides=1, use_bias=False)(inputs) 
                # x=keras.layers.Conv2D(self.channel, 3, strides=1,padding='same', use_bias=False)(x) 
                # x=keras.layers.Conv2D(self.channel, 1, strides=1, use_bias=False)(x) 
...
#            exec('x_{}={}'.format(i,keras.layers.Input((65,1,1,))),globals())
            ...
            multi_scale_1=BottleneckLIP_LIP(input_shape[2],1)(x_0)
            multi_scale_2=BottleneckLIP_LIP(input_shape[2],2)(x_0)
            multi_scale_3=BottleneckLIP_LIP(input_shape[2],3)(x_0)
            multi_scale_4=BottleneckLIP_LIP(input_shape[2],4)(x_0)
            multi_scale_5=BottleneckLIP_LIP(input_shape[2],5)(x_0)
