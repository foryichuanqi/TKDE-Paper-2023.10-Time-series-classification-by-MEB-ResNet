...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 14 15:48:39 2022
...
nb_epochs = 250
run_times=10
method_name='R_2_multi_scale_max_Pool_RESNET_MTSC'
    ...
    for i in range(run_times):
        ...
        # 去替代标准的 layers.Dense()
            ...
            # def call(self, inputs):
                # print(inputs.shape)
                # print('inputs')                
                e_x=Lambda(lambda x: K.exp(x))(inputs)
                e_x=Lambda(lambda x: K.sum(x,axis=3, keepdims=True))(e_x)  ############CHANNAL  AXIS=3
                # print(e_x.shape)
                # print('vvvv')
                e_x=Lambda(lambda x: K.clip(x, float(0), float('inf')))(e_x)
                e_x_inputs=keras.layers.multiply([inputs,e_x])  
                e_x_inputs_ave1=self.ave1(e_x_inputs)
                # print(e_x.shape)
                # print('vvvv')
                e_x_ave2=self.ave2(e_x)
                e_x_ave2=Lambda(lambda x: 1/x)(e_x_ave2)
                output=keras.layers.multiply([e_x_inputs_ave1,e_x_ave2]) 
                output=Lambda(lambda x: K.clip(x, float(0), float('inf')))(output)
...
#            exec('x_{}={}'.format(i,keras.layers.Input((65,1,1,))),globals())
            ...
            multi_scale_1=keras.layers.MaxPooling2D(1,1)(x_0)
            multi_scale_2=keras.layers.MaxPooling2D((2,1))(x_0)
            multi_scale_3=keras.layers.MaxPooling2D((3,1))(x_0)
            multi_scale_4=keras.layers.MaxPooling2D((4,1))(x_0)
            multi_scale_5=keras.layers.MaxPooling2D((5,1))(x_0)
